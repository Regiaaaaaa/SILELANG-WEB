...
@section('title', 'Pengajuan Barang User')

@section('content')
<div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Daftar Pengajuan Barang Lelang</h1>
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-center space-x-2">
                        ...
                        <!-- Tombol Verifikasi -->
                        <button onclick="verifikasiBarang('{{ $item->id }}')"
                            class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm">
                            Setujui
                        </button>
                        <!-- Tombol Tolak -->
                        <button onclick="tolakBarang('{{ $item->id }}')"
                ...
                <tr>
                    <td colspan="7" class="px-4 py-8 text-center text-gray-500">
                        Tidak ada pengajuan barang lelang yang menunggu verifikasi.
                    </td>
                </tr>
                @endforelse
            </tbody>
        ...
        <div class="mt-3">
            ...
            <div id="detailContent" class="hidden">
                ...
                <div class="flex justify-end space-x-2 mt-6 pt-4 border-t">
                    ...
                    <button id="btnSetujuiModal" onclick="verifikasiBarangDariModal()" 
                        class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
                        Setujui
                    </button>
                    <button id="btnTolakModal" onclick="tolakBarangDariModal()" 
...
<!-- Modal untuk Tolak Barang -->
<div id="modalTolak" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <h3 class="text-lg font-bold text-gray-900 mb-4">Tolak Pengajuan Barang</h3>
            <textarea id="catatanTolak" rows="4" 
            ...
            <div class="flex justify-end space-x-2 mt-4">
                ...
                <button onclick="konfirmasiTolak()" 
                    class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">
                    Tolak Barang
                </button>
            </div>
...
<script>
    ...
    // Fungsi untuk verifikasi barang dari modal
    function verifikasiBarangDariModal() {
        if (currentBarangId) {
            verifikasiBarang(currentBarangId);
        }
    }

    // Fungsi untuk tolak barang dari modal
    function tolakBarangDariModal() {
        if (currentBarangId) {
            tutupModalDetail();
            tolakBarang(currentBarangId);
        }
    }

    // Fungsi untuk verifikasi/setujui barang
    // Update fungsi verifikasi barang di file pending lu (ganti yang lama)
function verifikasiBarang(id) {
    if (confirm('Yakin mau setujuin pengajuan barang ini?')) {
        const token = getAuthToken();
        
        if (!token) {
            alert('Token autentikasi ga ketemu. Login ulang deh.');
            return;
        }

        // Tampilkan loading state
        const button = document.querySelector(`button[onclick="verifikasiBarang('${id}')"]`);
        if (button) {
            button.disabled = true;
            button.innerHTML = 'Memproses...';
        }

        fetch(`/api/admin/barang-lelang/${id}/verifikasi`, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            alert(data.message || 'Barang berhasil disetujui dan dipindah ke aktif!');
            
            // Tutup modal kalau ada yang buka
            if (currentBarangId) {
                tutupModalDetail();
            }
            
            // Redirect ke halaman aktif atau reload
            if (confirm('Mau liat daftar pengajuan aktif sekarang?')) {
                window.location.href = '/admin/pengajuan-aktif'; // Sesuaikan dengan route lu
            } else {
                location.reload(); // Reload halaman pending
            }
        })
        .catch(err => {
            console.error('Error:', err);
            ...
            if (err.message.includes('401') || err.message.includes('Unauthenticated')) {
                alert('Sesi lu udah abis. Login ulang deh.');
                // window.location.href = '/login'; // Uncomment kalau perlu redirect ke login
            } else {
                alert('Ada error saat verifikasi barang: ' + err.message);
            }
        });
    }
}
    ...
    function konfirmasiTolak() {
        ...
        fetch(`/api/admin/barang-lelang/${barangIdTolak}/tolak`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
        ...
        .then(data => {
            alert(data.message || 'Barang berhasil ditolak!');
            tutupModalTolak();
            location.reload();
        })
        .catch(err => {
            ...
            if (err.message.includes('401') || err.message.includes('Unauthenticated')) {
                ...
                alert('Terjadi kesalahan saat menolak barang: ' + err.message);
            }
        });
    }
    ...
    document.getElementById('modalTolak').addEventListener('click', function(event) {
        if (event.target === this) {
            tutupModalTolak();
        }
    });
